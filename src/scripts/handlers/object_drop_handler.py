...
def DropHandler(player, object):
	...
	if u_object not in misc_objects.keys():
		...
		if u_object not in player.inventory:
			...
			if len(player.current_world[player.current_location]["objects"]) + 1 > LOCATION_PLACEMENT_SIZE:
				player.previous_error = LocationSpaceError(player)
				GameScreen(player)
			else:
				player.inventory.remove(u_object)
				player.current_world[player.current_location]["objects"].append(u_object)
				player.previous_error = f"You dropped the \"{object_name}\"."